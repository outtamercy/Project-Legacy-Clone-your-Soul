    ...
    while i < fxCount
        ...
        if fx
            if fx.IsDeleted()
                ...
                ; born == 0 = pre-sweeper orphan riding the save: IMMORTAL under
                ; the old born>0 rule, refires OnLoad every cell attach (the
                ; 1,430-error flood), and IS the persistent swirly. kill on sight.
                if born == 0.0 || (born > 0.0 && now - born > 15.0)
                    fx.DisableNoWait()
                    fx.Delete()
                    fxRefs[i] = None
...
Actor Function ConstructVesselFromRegistry()
    ...
    if diskName != ""
        ...
        if faceOk
            vessel.QueueNiNodeUpdate()  ; force face-tint resolve after a successful load
        else
...
bool Function DoBind()
    ...
    if diskName != ""
        ...
        if faceOk
            vessel.QueueNiNodeUpdate()  ; face snaps under the ghost shader — cure for bind black face
        else
            Debug.Notification("Project Legacy: Face load failed for " + diskName)
        endif
    endif
