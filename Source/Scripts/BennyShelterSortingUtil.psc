Scriptname BennyShelterSortingUtil extends Quest
...
Function SortItemsFromContainer(ObjectReference akContainerRef)
    ...
    Form[] RefItems = akContainerRef.GetContainerForms()
    Int itemCount = RefItems.Length
    Int ItemsSorted = 0
    Int i = 0
    ...
    While i < itemCount
        Form currentItem = RefItems[i]
        int currentItemCount = akContainerRef.GetItemCount(currentItem)
        bool Sorted = False
        ...
        If akContainerRef == PlayerRef
            isFavorited = Game.IsObjectFavorited(currentItem)
            isEquipped = PlayerRef.IsEquipped(currentItem)
            equippedRight = PlayerRef.GetEquippedWeapon() == currentItem
            equippedLeft = PlayerRef.GetEquippedWeapon(True) == currentItem
        EndIf
        ...
        If currentItem.HasKeyword(VendorItemKey) ; Keys don't get sorted, use quest item bool to skip sorting loop for efficiency
            isQuestItem = True
        EndIf

        If !isQuestItem ; If quest item sorting is allowed, this will always be false, can be used to skip loop in other cases as well
            If currentItem == Gold001 ; Gold Sorting
                If sortGold
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterGoldStrongboxRef)
                    Sorted = True
                EndIf
            ElseIf currentItem == SaltPile ; Salt Sorting
                If sortIngredient
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterSaltContainerRef)
                    Sorted = True
                EndIf
            ElseIf currentItem == DwarvenCenturionDynamo ; Centurion Dynamo Sorting
                If sortClutter
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterCenturionContainerRef)
                    Sorted = True
                EndIf
            ElseIf currentItem == BYOHFoodFlour ; Flour Sorting
                If sortFood
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterFlourChestRef)
                    Sorted = True
                EndIf
            ElseIf aaBennyConstructList.HasForm(currentItem) ; Construct cubes sorting
                If !isFavorited
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterConstructBarrelRef)
                    Sorted = True
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemOreIngot) ; Ore/Ingots Sorting
                If sortOre
                    If aaBennyOreList.HasForm(currentItem)
                        akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterOreChestRef)
                        Sorted = True
                    Else
                        akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterIngotChestRef)
                        Sorted = True
                    EndIf
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemBook) && !currentItem.HasKeyword(VendorItemScroll) ; Book Sorting
                If aaBennySortBook.GetValueInt() > 0
                    If aaBennyRecipesList.HasForm(currentItem) || currentItem.HasKeyword(VendorItemRecipe)
                        akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterRecipeChestRef)
                        Sorted = True
                    Else
                        akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterBookChestRef)
                        Sorted = True
                    EndIf
                EndIf
                ...
                If sortFood
                    If !isFavorited
                        If aaBennyLiquorList.HasForm(currentItem)
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterLiquorChestRef)
                            Sorted = True
                        ElseIf aaBennyCheeseList.HasForm(currentItem)
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterCheeseChestRef)
                            Sorted = True
                        ElseIf aaBennyCookedMeatsList.HasForm(currentItem)
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterCookedMeatBarrelRef)
                            Sorted = True
                        ElseIf aaBennyRawMeatsList.HasForm(currentItem)
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterRawMeatBarrelRef)
                            Sorted = True
                        ElseIf aaBennyFruitsVegetablesList.HasForm(currentItem)
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterVegetablesBarrelRef)
                            Sorted = True
                        ElseIf aaBennySoupsList.HasForm(currentItem)
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterStewBarrelRef)
                            Sorted = True
                        ElseIf !currentItem.HasKeyword(VendorItemFoodRaw)
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterCookedFoodBarrelRef)
                            Sorted = True
                        Else
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterRawFoodBarrelRef)
                            Sorted = True
                        EndIf
                    EndIf
                EndIf
                    ...
                    If !isFavorited
                        If !isEquipped
                            If sortClothing && isClothing && !isArmor
                                akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterClothingChestRef)
                                Sorted = True
                            Else
                                akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterArmorChestRef)
                                Sorted = True
                            EndIf
                        EndIf
                    EndIf
                ...
                If sortWeapons
                    If !isFavorited
                        If !equippedRight && !equippedLeft
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterWeaponChestRef)
                            Sorted = True
                        EndIf
                    EndIf
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemArrow) ; Ammo Sorting
                If sortAmmo
                    If !isFavorited
                        If !isEquipped
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterAmmoChestRef)
                            Sorted = True
                        EndIf
                    EndIf
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemGem) ; Gem Sorting
                If sortGem
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterGemStrongboxRef)
                    Sorted = True
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemSoulGem) ; Soul Gem Sorting
                If sortSoulGem
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterSoulgemStrongboxRef)
                    Sorted = True
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemIngredient) ; Ingredient Sorting
                If sortIngredient
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterIngredientStrongboxRef)
                    Sorted = True
                EndIf
            ...
            ElseIf isJewelry ; Jewelry Sorting
                If sortJewelry
                    If !isFavorited
                        If !isEquipped
                            akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterJewelryStrongboxRef)
                            Sorted = True
                        EndIf
                    EndIf
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemPoison) ; Poison Sorting
                If sortPoisons
                    If !isFavorited
                        akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterPoisonChestRef)
                        Sorted = True
                    EndIf
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemPotion) ; Potion Sorting
                If sortPotions
                    If !isFavorited
                        akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterPotionChestRef)
                        Sorted = True
                    EndIf
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemScroll) ; Scroll Sorting
                If sortScroll
                    If !isFavorited
                        akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterScrollStrongboxRef)
                        Sorted = True
                    EndIf
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemClutter) || currentItem.HasKeyword(BYOHHouseCraftingCategorySmithing) ; Clutter Sorting
                If sortClutter
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterMiscChestRef)
                    Sorted = True
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemFirewood)
                If sortFirewood
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterFirewoodChestRef)
                    Sorted = True
                EndIf
            ElseIf currentItem.HasKeyword(VendorItemAnimalHide) || currentItem.HasKeyword(VendorItemAnimalPart)
                If sortLeather
                    akContainerRef.RemoveItem(currentItem, currentItemCount, True, aaBennyShelterLeatherChestRef)
                    Sorted = True
                EndIf
            Else
