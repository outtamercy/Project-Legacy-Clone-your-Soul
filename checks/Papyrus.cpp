...
namespace ProjectLegacy::Papyrus {
    ...
    bool PerformBind(RE::Actor* vessel, int32_t slot, std::string slotName, std::string echoName) {
        ...
        // sex: direct copy, and it now lands BEFORE papyrus SetRace by call
        // order, so the race-switch event never sees the wrong sex
        ...
        // ---- payload: file name from the caller, registry as backup ----
