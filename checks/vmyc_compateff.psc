...
Event OnUpdate()
	If GetModByName("EFFCore.esm") != 255
		DoEFFPanelUpdate()
	ElseIf GetModByName("XFLMain.esm") != 255
		DoXFLPanelUpdate()
	EndIf
EndEvent
...
;FOR OLDER VERSIONS!
	...
	If GetModByName("XFLMain.esm") != 255 && GetModByName("XFLPanel.esp") != 255
		SKI_WidgetManager WidgetManager = GetFormFromFile(0x00000824,"SkyUI_SE.esp") as SKI_WidgetManager
...
Function DoEFFPanelUpdate()
	...
	;This updates the character's name in EFF's widget, if it's installed
	;Expired has said he'll add a ModEvent we can use to do this in future, which should be much simpler
	Debug.Trace("MYC/CompatEFF: Updating EFFPanel names!")
	Int i = 0
	If GetModByName("EFFCore.esm") != 255
		SKI_WidgetManager WidgetManager = GetFormFromFile(0x00000824,"SkyUI_SE.esp") as SKI_WidgetManager
