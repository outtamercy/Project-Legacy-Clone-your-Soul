...
Bool Function CheckDependencies()
	...
	Debug.Trace("MYC: Skyrim Engine Extender is version " + SKSE.GetPluginVersion("skee"))
	;Debug.MessageBox("SKSE version is " + fSKSE)
	If fSKSE < 2.0017
		Debug.MessageBox("Familiar Faces\nSKSE is missing or not installed correctly. This mod requires SKSE 2.0.17 or higher, but the current version is " + fSKSE + ".\nThe mod will now shut down.")
		Return False
	Else
		;Proceed
	EndIf
	...
	If SKSE.GetPluginVersion("skee") < 1
		Debug.MessageBox("Familiar Faces\nThe SKSE plugin SKEE.dll is missing or not installed correctly. This mod requires RaceMenu Special Edition.\nThe mod will now shut down.")
		Return False
	Else
		;Proceed
	EndIf
	If SKSE.GetPluginVersion("skee") >= 1 && NIOverride.GetScriptVersion() > 1
